...
void Encoder_Controller()
{
   newTuning=tuning.read()/4 ;
     if(newTuning!=oldTuning)
     {
      ...
      {
        Freq=Freq+Step;
        Serial.print("tuning encoder rotated right");
      }
     if(newTuning<oldTuning)
      {
        Freq=Freq-Step;
        Serial.print("tuning encoder rotated left");
     ...
     if(newFilter!=oldFilter)
     {
      if(newFilter>oldFilter)
      {
        if(filterCenter<2500)
        {
          ...
          SetFilter();
          Serial.print("CenterFrequenncy encoder rotated right");
        }
        else
        {
         filterCenter=2500;
      ...
      if(newFilter<oldFilter)
      {
       ...
       { 
         ...
         filterCenter=filterCenter-10;
          SetFilter();
          Serial.print("CenterFrequenncy encoder rotated Left");
       }
       else
       {
        filterCenter=250;
     ...
     if(newBandwidth!=oldBandwidth)
     {
      if(newBandwidth>oldBandwidth)
      {
        if(filterBandwidth<5000)
        {
            ...
            SetFilter();
            Serial.print("FilterBandwidth rotated right");
        }
        else
        {
